...
class HSimpleFC{
	...
	public function VerHSimpleFC(){
		...
		echo '<h3>Imagen Principal:</h3>';
			...
			if($respuesta != ""){
				...
				echo '<img src="Vistas/img/hs2" class="img-thumbnail" width="250px">';
	...
	//Editar HSimple
	public function EditarHSimpleFC(){

		$tablaBD = "hsimple2";

		$id = "1";

		$respuesta = HSimple2M::EditarHSimpleFM($tablaBD, $id);
	...
	//Actualizar HS
	public function ActualizarHSimpleFC(){
		...
		if(isset($_POST["precioE"])){
			...
			if(isset($_FILES["imagenE"]["tmp_name"]) && !empty($_FILES["imagenE"]["tmp_name"])){
				...
				if($_FILES["imagenE"]["type"] == "image/jpeg"){
					...
					$rutaImg = "Vistas/img/hs2".$nombre.".jpg";
				...
				if($_FILES["imagenE"]["type"] == "image/png"){
					...
					$rutaImg = "Vistas/img/hs2".$nombre.".png";
			...
			$respuesta = HSimple2M::ActualizarHSimpleFM($tablaBD, $datosC);

			if($respuesta == true){

				echo '<script>

				window.location = "hsimpleF";
